...
核心不再是“数字人运行时框架”，而是“AI 角色平台”。TTS、ASR、RAG、Workflow、ComfyUI 角色外观生成都是角色运行能力的一部分，不能反过来成为产品中心。Live2D 只作为历史兼容能力保留，后续不再作为主线投入。

## 2. Product Definition

平台要解决的问题：

- 用户可以通过 ComfyUI 文生图、图生图创建 AI 角色。
...
| Character Asset | 角色图像、ComfyUI 生成图、声音、背景等资产 | AvatarAsset |
...
| Provider | ComfyUI、LLM、TTS、ASR、Embedding 等服务商 | ProviderConfig |
...
- TTS：角色声音输出能力。
- ASR：角色语音输入能力。
- ComfyUI：角色外观生成和资产生产 Provider。
- Live2D：历史兼容表现层，不再作为后续主线能力。
...
- 旧 Avatar Runtime 作为历史兼容表现层存在，新外观生产转向 ComfyUI。
...
暂时不调用 `image_gen`。
图像生成能力后续改为 ComfyUI Provider / Workflow，不再使用 `image_gen`。当前阶段只做接口和配置设计，不依赖真实图片生成结果推进开发。
...
- 角色库是核心入口，Avatar 只是历史兼容表现层，ComfyUI 是后续外观生成方向。
