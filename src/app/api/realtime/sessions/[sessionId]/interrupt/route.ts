...
export async function POST(
  ...
  try {
    ...
    await store.set(interrupted, ttlSeconds);
    const event = await appendRealtimeSessionEvent(store, {
      payload: {
        reason: interrupted.interruptReason,
    ...
    return jsonData({
      event: presentRealtimeSessionEvent(event),
